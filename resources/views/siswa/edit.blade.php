...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <div>
        <h1 class="h3 mb-1 text-gray-800 font-weight-bold">Edit Data Siswa</h1>
        <p class="text-muted small mb-0">Perbarui data siswa: <strong>{{ $siswa->nama }}</strong> (NIS: {{ $siswa->nis }})</p>
    </div>
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3 bg-white">
                <h6 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-user-edit me-1"></i> Formulir Perubahan Data Siswa
                </h6>
            </div>

            <div class="card-body">
                <form action="{{ route('siswa.update', $siswa->id) }}" method="POST" enctype="multipart/form-data" id="formEditSiswa">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="email" name="email" id="email" 
                               ...
                               required>
                        <small class="form-text text-muted">Pastikan format email valid.</small>
                        @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <div class="row align-items-center g-3">
                            <div class="col-auto">
                                <img id="previewImage" src="{{ $siswa->foto_url }}" 
                                     alt="{{ $siswa->nama }}" class="rounded border shadow-sm" style="width: 75px; height: 75px; object-fit: cover;">
                            </div>
                            <div class="col">
                                ...
                                <small class="form-text text-muted d-block mt-1" id="fileHelpText">
                                    <i class="fas fa-info-circle me-1"></i> Biarkan kosong jika tidak mengubah foto. Format: <strong>JPG</strong> dan <strong>PNG</strong> (maksimal <strong>100 KB</strong>).
                                </small>
    ...
    function validateAndPreviewImage(input) {
        ...
        if (!allowedTypes.includes(file.type)) {
            Swal.fire({
                ...
                text: 'Format yang diizinkan hanya JPG dan PNG.'
            });
            input.value = '';
            preview.src = "{{ $siswa->foto_url }}";
            return;
        ...
        if (file.size > maxSizeBytes) {
            ...
            Swal.fire({
                ...
                text: 'Ukuran foto adalah ' + (file.size / 1024).toFixed(1) + ' KB. Batas maksimal yang diizinkan adalah 100 KB.'
            });
            input.value = '';
            preview.src = "{{ $siswa->foto_url }}";
            return;
